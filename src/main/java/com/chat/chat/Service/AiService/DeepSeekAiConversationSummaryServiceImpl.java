...
import com.chat.chat.DTO.AiConversationMessageDTO;
...
@Service
@Validated
public class DeepSeekAiConversationSummaryServiceImpl implements AiConversationSummaryService {

    private static final Logger LOGGER = LoggerFactory.getLogger(DeepSeekAiConversationSummaryServiceImpl.class);
    ...
    @Override
    public AiConversationSummaryResponseDTO resumirConversacion(AiConversationSummaryRequestDTO request) {
        ...
        List<AiConversationMessageDTO> sanitizedMessages = sanitizeMessages(request == null ? null : request.getMensajes());
        if (sanitizedMessages.isEmpty()) {
            return failure("AI_SUMMARY_EMPTY", "No hay mensajes suficientes para resumir.");
        }

        String userContent = buildConversationContext(sanitizedMessages);
    ...
    private void validateChatAccess(AiConversationSummaryRequestDTO request, Long userId, String chatType) {
    ...
    private List<AiConversationMessageDTO> sanitizeMessages(List<AiConversationMessageDTO> messages) {
        if (messages == null || messages.isEmpty()) {
            return List.of();
        }
        int maxMessages = Math.max(1, aiProperties.getMaxSummaryMessages());
        int start = Math.max(0, messages.size() - maxMessages);
        List<AiConversationMessageDTO> sanitized = new ArrayList<>();
        for (int i = start; i < messages.size(); i++) {
            AiConversationMessageDTO message = messages.get(i);
            if (message == null) {
                continue;
            }
            ...
            AiConversationMessageDTO current = new AiConversationMessageDTO();
    ...
    private String buildConversationContext(List<AiConversationMessageDTO> messages) {
        StringBuilder builder = new StringBuilder("Conversacion reciente:\n");
        for (AiConversationMessageDTO message : messages) {
    ...
    private String buildLine(AiConversationMessageDTO message) {
    ...
    private AiConversationSummaryResponseDTO success(String summary) {
        AiConversationSummaryResponseDTO response = new AiConversationSummaryResponseDTO();
    ...
    private AiConversationSummaryResponseDTO failure(String code, String message) {
        AiConversationSummaryResponseDTO response = new AiConversationSummaryResponseDTO();
